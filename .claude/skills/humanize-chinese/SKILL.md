...
检测和改写中文 AI 生成文本的完整工具链。Python 包 `humanize_cn`，统一 CLI 入口 `./humanize`。
...
```
src/humanize_cn/           # Python 包
├── config.py              # 配置加载
├── detection/             # 检测模块
│   ├── detect.py          # 通用检测（20+ 规则 + 8 统计特征）
│   └── academic.py        # 学术检测（11 维度）
├── models/                # 模型模块
│   ├── ngram.py           # N-gram 统计特征
│   ├── bert_detector.py   # BERT 检测器
│   ├── semantic_guard.py  # 语义守卫
│   └── restructure.py     # 句式重组
├── rewriting/             # 改写模块
│   ├── humanize.py        # 通用改写（同义词/连接词/噪声）
│   ├── paragraph.py       # 段落级改写
│   ├── targeted.py        # 定向改写（低频 n-gram 注入）
│   └── llm_rewrite.py     # LLM API 改写（可选）
├── interfaces/
│   └── cli.py             # 统一 CLI
└── data/                  # 数据文件（模型权重、配置、频率表）
```
...
```bash
# 需要先安装包或设置 PYTHONPATH
pip install -e ".[bert]"          # 安装包（含 BERT 依赖）
# 或
export PYTHONPATH=src

# 检测 + 详细
./humanize detect 文本.txt -v
# 快速改写（18× 速度）
./humanize rewrite 文本.txt -o 改后.txt --quick
# 学术降重 + 双评分对比
./humanize academic 论文.txt -o 改后.txt --compare
# 风格转换
./humanize style 文本.txt --style xiaohongshu
# 前后对比
./humanize compare 文本.txt -a
```

### Python API

```python
from humanize_cn import detect_patterns, calculate_score, humanize_paragraph
from humanize_cn.detection.academic import detect_academic, humanize_academic
...
rewritten = humanize_academic(text)
```
...
所有检测模式和替换规则在 `src/humanize_cn/data/patterns_cn.json`，可自定义：
- 添加新 AI 词汇
- 调整权重
- 增加替换规则
- 修改正则匹配
