...
def main():
    ...
    lr_result = None if args.rule_only else (compute_lr_score(text, scene=args.scene) if compute_lr_score else None)

    # BERT 检测器（用户训练的 ONNX 模型）
    bert_result = None
    try:
        from ..models.bert_detector import get_bert_score
