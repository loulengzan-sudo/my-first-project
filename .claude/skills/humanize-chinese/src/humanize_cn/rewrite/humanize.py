...
def _load_protected_terms(scene):
    ...
    """
    pt_file = os.path.join(SCRIPT_DIR, '..', 'data', 'protected_terms.json')
...
from ..models.ngram import analyze_text as ngram_analyze
...
def pick_best_replacement(sentence, old, candidates):
    ...
    try:
        from ..models.ngram import compute_perplexity
    except ImportError:
        compute_perplexity = None
    if compute_perplexity is None:
        return random.choice(candidates) if candidates else ''
...
ACADEMIC_BLACKLIST_CANDIDATES = {
    ...
    '识破', '觉察', '察觉', '看出', '拆解', '宛若',
    '取用', '守住', '办到', '带动', '处置', '供给', '开展',
    '最要紧', '拿上', '呈上', '催动', '助推', '留住',
    # ── 不通顺的动词替换 ──
    '凭借',  # 替换"通过/利用" → "凭借ORM"不通顺
    '经由',  # 替换"通过" → "经由ORM"不通顺
    '依靠',  # 替换"通过" → "依靠ORM"不通顺
    '扶持',  # 替换"支持" → "扶持跨浏览器"不通顺
    '达成',  # 替换"实现" → "达成安全"不通顺
    '引用',  # 替换"采用" → "引用Python"不通顺
    '沿用',  # 替换"采用/使用" → "沿用Python"不通顺
    '给出',  # 替换"提供" → "给出支持"不通顺
    '拿出',  # 替换"提供" → 不通顺
    '动用',  # 替换"使用/采取/利用" → "动用Django"不通顺
    '借用',  # 替换"利用" → "借用ORM"不通顺
    '使出',  # 替换"采取" → 不通顺
    '创设',  # 替换"建立" → "创设索引"不通顺
    '招来',  # 替换"引起" → 不通顺
    ...
    '觉得',  # 替换"认为" → 不通顺
    '夯实',  # 替换"加强" → "夯实管理"不通顺
    '彰显',  # 替换"体现" → "彰显了"勉强可接受但风格不对
    '折射',  # 替换"反映/体现" → 不通顺
    '映射',  # 替换"反映" → "映射了"不通顺
    '着眼',  # 替换"关注" → 不通顺
    '在乎',  # 替换"关注" → 不通顺
    '糅合',  # 替换"结合" → "糅合Bootstrap"不通顺
    '缔造',  # 替换"创造" → "缔造平台"不通顺
    '营造',  # 替换"创造" → "营造系统"不通顺
    '刻画',  # 替换"描述" → "刻画系统"不通顺
    '勾勒',  # 替换"描述" → 不通顺
    ...
    '算是',  # 替换"属于" → 不通顺
    '投身',  # 替换"参与" → 不通顺
    '打造',  # 替换"创造" → "打造系统"勉强可接受但口语化
    '力陈',  # 替换"强调" → 不通顺
    '重申',  # 替换"强调" → 语义不同
    '拼成',  # 替换"组成" → 不通顺
    '铸成',  # 替换"形成" → "铸成数据"不通顺
    '仰赖',  # 替换"基于" → 不通顺
    '衔接',  # 替换"结合" → "衔接Bootstrap"不通顺
    '触发',  # 替换"引起" → "触发问题"勉强可接受但语义不同
    '激起',  # 替换"引起" → 不通顺
    # ── 名词/形容词 - 口语化 ──
    '本事', '家底', '本钱', '档次', '段位', '地带', '招数', '打法',
    '麻烦', '症结', '亮点', '好处', '苗头', '势头', '门槛',
    '成堆的', '最要紧的', '海量',
    '要紧', '紧要',  # 替换"重要" → "要紧作用"不通顺
    '功底', '才干',  # 替换"能力" → 不通顺
    '长处', '强项',  # 替换"优势" → "长处是"口语化
    '靶心',  # 替换"目标" → 不通顺
    '素材',  # 替换"内容" → "素材包括"不通顺
...
def _filter_candidates_for_scene(word, candidates, scene):
    ...
    NOTE: 学术场景下，若所有候选都被过滤，返回空列表（不替换），
    而非返回原始候选（会导致黑名单词被选中）。
    """
...
NOISE_ACADEMIC_CATEGORIES = ['hedging', 'uncertainty']
...
def _load_bigram_freq():
    ...
    try:
        from ..models.ngram import _load_freq
    except ImportError:
        return {}
    freq = _load_freq()
...
def cap_transition_density(text, target=None):
    ...
    try:
        from ..models.ngram import _TRANSITION_PHRASES
    except ImportError:
        _TRANSITION_PHRASES = None
    if _TRANSITION_PHRASES is None:
        return text
...
def humanize(text, scene='general', aggressive=False, seed=None, best_of_n=DEFAULT_BEST_OF_N):
    ...
    if best_of_n and best_of_n > 1:
        try:
            from ..models.ngram import compute_lr_score
        except ImportError:
            compute_lr_score = None
        if compute_lr_score is None:
            return humanize(text, scene=scene, aggressive=aggressive,
                           seed=seed, best_of_n=None)
    ...
    try:
        from ..models.restructure import deep_restructure
    except ImportError:
...
def main():
    ...
    if args.style:
        try:
            from .style import apply_style as _apply_style
            result = _apply_style(result, args.style, humanize_first=False, seed=args.seed)
        except ImportError:
            print(f'警告: 未找到风格转换模块', file=sys.stderr)
