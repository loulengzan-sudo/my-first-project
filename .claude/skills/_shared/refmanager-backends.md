...
if [ -z "$ZOTERO_DIR" ]; then
  ...
  # Fallback: deep search via find if glob-based detection failed (handles special chars in paths)
  if [ -z "$ZOTERO_DIR" ] && [ -d "$HOME/Library/CloudStorage" ]; then
    _zfound=$(find -L "$HOME/Library/CloudStorage" -maxdepth 8 -name "zotero.sqlite" 2>/dev/null | head -1)
    if [ -n "$_zfound" ]; then
      ZOTERO_DIR=$(dirname "$_zfound")
    fi
    unset _zfound
  fi
fi
