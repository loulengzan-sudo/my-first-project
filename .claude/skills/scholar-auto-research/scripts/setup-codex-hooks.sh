...
#   with spaces (including Dropbox project paths) FAILS OPEN under Codex (it word-splits
...
#   (references/data-safety-runtime.md).
#
...
command -v python3 >/dev/null 2>&1 || {
  echo "FAIL: required runtime command unavailable: python3" >&2
  exit 1
}
[ -r "$SKILL_DIR/scripts/runtime-preflight.py" ] || {
  echo "FAIL: required runtime dependency missing/unreadable: scripts/runtime-preflight.py" >&2
  exit 1
}
python3 "$SKILL_DIR/scripts/runtime-preflight.py" --skill-dir "$SKILL_DIR" --quiet || exit 1

ADAPTER_REL="$SKILL_DIR/scripts/gates/codex-pretooluse-hook.sh"
