#!/usr/bin/env bash
# codex-trigger-phase14.sh — auto-research Phase 14 codex cross-model review gate.
# Self-contained within scholar-auto-research; calls the sibling
# codex-trigger-check.sh in this same gates/ dir.
#
# Called by auto-research-verify.sh `run_external_gate("codex-trigger-phase14.sh", ...)`.
...
if [ "$#" -ne 1 ]; then
  ...
  echo "DETAIL: usage: codex-trigger-phase14.sh <project_dir>"
  exit 2
fi
