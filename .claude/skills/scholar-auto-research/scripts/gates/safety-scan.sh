...
# ── Category 11: Restricted Dataset Fingerprint ──────────────────────────
# Run before Presidio. Presidio can return GREEN on renamed restricted
# datasets because their headers contain no direct identifiers; a DUA
# fingerprint is a scanner-level RED regardless of NER output.
DUA_REGISTRY="$SCRIPT_DIR/dua-fingerprints.json"
if [ ! -r "$DUA_REGISTRY" ]; then
  echo "RED: required DUA fingerprint registry missing/unreadable: $DUA_REGISTRY"
  exit 1
fi
if ! command -v python3 >/dev/null 2>&1; then
  echo "RED: required local DUA registry parser unavailable: python3"
  exit 1
fi
DUA_ROWS=$(python3 - "$DUA_REGISTRY" <<'PY'
import json, sys
...
for d in datasets:
    if not isinstance(d, dict) or not str(d.get("name") or "").strip():
        raise SystemExit(1)
    vals = [
        d.get("name", ""), d.get("owner", ""), d.get("restriction", ""),
        "|".join(d.get("filename_tokens") or []),
        "|".join(",".join(sig) for sig in (d.get("column_signatures") or [])),
    ]
    print("\t".join(str(v).replace("\t", " ").replace("\n", " ") for v in vals))
PY
) || {
  echo "RED: required DUA fingerprint registry is malformed or empty: $DUA_REGISTRY"
  exit 1
}
if [ -n "$DUA_ROWS" ]; then
  BASENAME_LOWER="$(basename "$LOWER_FILE")"

  HEADER_LINE=""
  if [ "$BINARY_SHORTCUT" = 0 ]; then
    IFS= read -r HEADER_LINE < "$FILE" 2>/dev/null || HEADER_LINE=""
  fi
  HEADER_LOWER="$(printf '%s' "${HEADER_LINE:-}" | tr -d '\r' | tr '[:upper:]' '[:lower:]')"

  while IFS=$'\t' read -r DS_NAME DS_OWNER DS_RESTRICT DS_TOKENS DS_SIGS; do
    [ -z "${DS_NAME:-}" ] && continue
    MATCHED=0
    MATCH_REASON=""
    ...
    if [ -n "${DS_TOKENS:-}" ]; then
      WRAPPED_TOKENS=""
      IFS='|' read -ra _TOKS <<< "$DS_TOKENS"
      for TOK in "${_TOKS[@]}"; do
        [ -z "$TOK" ] && continue
        if [ -n "$WRAPPED_TOKENS" ]; then WRAPPED_TOKENS="${WRAPPED_TOKENS}|"; fi
        WRAPPED_TOKENS="${WRAPPED_TOKENS}(^|[^a-z0-9])(${TOK})([^a-z0-9]|$)"
      done
      if [ -n "$WRAPPED_TOKENS" ] && printf '%s' "$BASENAME_LOWER" | grep -qE "$WRAPPED_TOKENS" 2>/dev/null; then
        MATCHED=1
        MATCH_REASON="filename token"
      fi
    fi

    if [ "$MATCHED" = 0 ] && [ -n "${HEADER_LOWER:-}" ] && [ -n "${DS_SIGS:-}" ]; then
      IFS='|' read -ra _SIGS <<< "$DS_SIGS"
      for SIG in "${_SIGS[@]}"; do
        [ -z "$SIG" ] && continue
        IFS=',' read -ra _COLS <<< "$SIG"
        SIG_OK=1
        for COL in "${_COLS[@]}"; do
          COL_LC="$(printf '%s' "$COL" | tr '[:upper:]' '[:lower:]')"
          if ! printf '%s' "$HEADER_LOWER" | grep -qE "(^|[^a-z0-9_])${COL_LC}([^a-z0-9_]|$)" 2>/dev/null; then
            SIG_OK=0
            break
          fi
        done
        if [ "$SIG_OK" = 1 ]; then
          MATCHED=1
          MATCH_REASON="column signature [${SIG}]"
          break
        fi
      done
    fi
    ...
    if [ "$MATCHED" = 1 ]; then
      ...
      ISSUES="${ISSUES}\n  RED: Restricted dataset fingerprint — ${DS_NAME} (${DS_OWNER}); detected via ${MATCH_REASON}; ${DS_RESTRICT}"
    fi
  done <<< "$DUA_ROWS"
fi
...
if [ -f "$PRESIDIO_SCRIPT" ] && python3 -c "from presidio_analyzer import AnalyzerEngine; AnalyzerEngine()" 2>/dev/null; then
  ...
  if echo "$PRESIDIO_OUT" | grep -q "^RED:"; then
    ...
    PRESIDIO_EXIT=0
  elif echo "$PRESIDIO_OUT" | grep -q "^NOTE:"; then
    # Deliberate skip (e.g., PDF / Office docs where Presidio NER on
    # binary-as-text is pathologically slow). Fall through to the
    # regex backend quietly — no warning, since this is expected.
    PRESIDIO_OUT=""
    PRESIDIO_EXIT=99
  else
